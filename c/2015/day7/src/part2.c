...
int main(void) {
  ...
  /* PART 2 SPECIFIC PIECE */
  char *name = "b";
  wire_t *wire_b = (wire_t *)malloc(sizeof(wire_t));
  wire_b->name = malloc(sizeof(char) * strlen(name));
  strncpy(wire_b->name, name, strlen(name));
  /*wire_b->name = name;*/
  wire_b->signal = 16076;
  linked_list_add(&wires, wire_b, sizeof(wire_t));
