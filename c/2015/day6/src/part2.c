...
size_t calculate_light_brightness(int ***lights, size_t len) {
  size_t number_of_lights_on = 0;

  for (size_t i = 0; i < len; ++i) {
    for (size_t j = 0; j < len; ++j) {
      number_of_lights_on += (*lights)[i][j];
    }
  }
...
int main(void) {
  ...
  if ((light_brightness = calculate_light_brightness(&lights, LIGHTS_LEN)) ==
      (size_t)AOC_ERR) {
